...
model_path = os.path.join(base_dir, 'models', 'time_series_model.h5')
...
try:
    if not os.path.exists(model_path) or not os.path.exists(threshold_path):
        raise FileNotFoundError("Model or threshold file not found.")
    model = load_model(model_path)
    anomaly_threshold = np.load(threshold_path)
    print("Model and threshold loaded successfully.")
except Exception as e:
    print(f"CRITICAL ERROR: Could not load model files. Please ensure 'train_model.py' has been run successfully.")
    print(f"Details: {e}")
    model = None
...
demo = gr.Interface(
    ...
    title="Predictive Maintenance Anomaly Detector"
)
